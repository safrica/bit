...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Bitcoin: starting difficulty is 1 / 2^12
...
{
    // Bitcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Bitcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Bitcoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Bitcoin: 2.5 minutes
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Bitcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
unsigned char pchMessageStart[4] = { 0xf3, 0xc2, 0xb3, 0xd1 }; // Bitcoin: increase each by adding 2 to bitcoin's value.
